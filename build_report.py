...
add_para('电信行业客户流失问题日益严峻，新客户获取成本远高于存量客户维护成本。本项目构建了一套完整的"流失预警+客户分群"双模块分析系统，基于Kaggle Telco Customer Churn数据集（7,043条记录，21个特征），综合运用数据清洗、RFM与WOE特征工程、折内SMOTE交叉验证、四种分类模型对比（逻辑回归、随机森林、XGBoost、LightGBM）、SHAP可解释性分析以及K-Means/DBSCAN聚类分析等方法。')
add_para('实验结果表明，在真实不平衡测试集上，随机森林模型取得最佳综合性能（ROC-AUC = 0.8386，Recall = 0.7086，F1 = 0.6295）。K-Means聚类成功将客户分为4个具有显著差异的群体，其中高危群体（Cluster 2）流失率高达60.9%，高价值群体（Cluster 1）月均消费$95且流失率仅21.3%。基于SHAP分析和聚类画像，提出了合同类型优化、增值服务捆绑、支付方式引导等六大方向的业务策略建议，为电信运营商的客户留存决策提供了数据驱动的支撑。')
...
add_bullet('有监督学习模块：对比逻辑回归、随机森林、XGBoost和LightGBM四种分类算法，通过训练集SMOTE处理类别不平衡，并在交叉验证中使用折内SMOTE Pipeline避免验证集泄漏，以ROC-AUC和Recall为核心指标，选出最佳流失预测模型。')
...
add_para('评估指标包括Accuracy（准确率）、Precision（精确率）、Recall（召回率）、F1-Score、ROC-AUC和PR-AUC六项。其中ROC-AUC是模型区分能力的综合指标，Recall直接反映流失客户的识别率，PR-AUC在不平衡场景下比ROC-AUC更具参考价值。验证方法采用5折分层交叉验证（StratifiedKFold, shuffle = True），数据按80%训练/20%测试分层划分（stratify = y）。交叉验证使用imblearn Pipeline在每个训练折内部执行SMOTE，测试集保持原始分布。随机种子固定为42，确保实验完全可复现。')
...
add_para('本实验包含三个层次的对比：（1）模型层面——逻辑回归 vs 随机森林 vs XGBoost vs LightGBM；（2）特征与不平衡处理层面——Base、Base+RFM、Base+RFM+WOE、Base+RFM+WOE+SMOTE四组消融对比；（3）聚类层面——K-Means（固定簇数）vs DBSCAN（密度自适应）。')

add_heading('5.2 5折交叉验证结果', level=2)
add_para('使用5折分层交叉验证评估模型稳定性。为避免数据泄漏，SMOTE被封装在imblearn Pipeline中，仅在每个训练折内部拟合与重采样，验证折保持原始分布。结果如表3所示。', indent=False)
...
add_para('在折内SMOTE交叉验证下，随机森林以ROC-AUC = 0.8393位居第一，逻辑回归紧随其后（ROC-AUC = 0.8349）。与旧的“先SMOTE再交叉验证”口径相比，该结果更接近真实泛化表现，也更符合无数据泄漏的实验规范。')
...
add_para('在真实不平衡数据上，随机森林以ROC-AUC = 0.8386和Recall = 0.7086取得综合最佳表现。这意味着模型能够成功识别70.9%的流失客户，同时保持56.6%的精确率。逻辑回归虽然整体准确率较低，但其Recall达到0.6979，与随机森林接近，说明线性模型在RFM和WOE特征工程的加持下仍具有竞争力。XGBoost和LightGBM的Accuracy略高，但Recall明显低于随机森林，不符合流失预警场景对召回率的偏好。')
...
add_para('从混淆矩阵可以看出（见附录experiments/confusion_matrices.png），四种模型对非流失客户的识别准确率较高（真负例多），但对流失客户存在一定程度的漏检（假负例）。以随机森林为例，374个真实流失客户中约265个被正确识别（Recall = 70.9%），约109个被漏检。这是不平衡分类的典型特征——模型倾向于预测多数类。在实际业务中，可通过调整分类阈值来平衡精确率与召回率。')

add_heading('5.6 特征重要性分析', level=2)
add_para('随机森林模型的特征重要性排序显示（见附录experiments/feature_importance.png），Contract（合同类型）是最重要的预测特征，其次是R_Score、MonthlyCharges、tenure和TotalCharges。RFM综合评分RFM_Total和WOE编码特征也进入了Top-10。')

add_heading('5.7 消融实验', level=2)
add_para('为验证RFM、WOE和SMOTE各模块的贡献，使用随机森林在同一训练/测试划分上进行消融对比，结果如表5所示。', indent=False)

add_table(
    ['实验条件', 'Accuracy', 'Precision', 'Recall', 'F1', 'ROC-AUC', 'PR-AUC'],
    [
        ['Base', '0.8013', '0.6632', '0.5107', '0.5770', '0.8424', '0.6543'],
        ['Base+RFM', '0.7984', '0.6630', '0.4893', '0.5631', '0.8407', '0.6557'],
        ['Base+RFM+WOE', '0.7956', '0.6536', '0.4893', '0.5596', '0.8386', '0.6475'],
        ['Base+RFM+WOE+SMOTE', '0.7786', '0.5662', '0.7086', '0.6295', '0.8386', '0.6509'],
    ]
)

add_para('消融结果显示，RFM与WOE在随机森林上没有带来明显AUC提升，但SMOTE显著改善了流失客户召回率：Recall从0.4893提升到0.7086，F1从0.5596提升到0.6295。该结果说明本任务的主要收益来自不平衡学习策略，而RFM/WOE更多提供可解释的业务特征补充。')
...
add_table(
    ...
    [
        ['1', 'PaymentMethod', '0.0566', '支付方式，电子支票支付者流失风险显著偏高'],
        ['2', 'MonthlyCharges', '0.0566', '月消费金额，高消费客户流失倾向略有增加'],
        ['3', 'RFM_Total', '0.0552', 'RFM综合评分，同时考虑了最近性、频率和消费价值'],
        ['4', 'tenure_WOE', '0.0552', '在网时长的证据权重编码，反映客户生命周期的非线性影响'],
        ['5', 'InternetService', '0.0272', '互联网服务类型，光纤用户流失率高于DSL用户'],
        ['6', 'MultipleLines', '0.0272', '多线路服务，与互联网服务类型高度相关'],
        ['7', 'R_Score', '0.0255', '最近性指标，新客户流失风险高于老客户'],
        ['8', 'TotalCharges', '0.0255', '累计消费总额，长期客户的总消费更高但流失率更低'],
    ]
)

add_para('SHAP分析与随机森林特征重要性相互印证，共同指出：PaymentMethod（支付方式）、MonthlyCharges（月费用）、RFM综合评分、tenure_WOE（在网时长证据权重）和InternetService（互联网服务类型）是驱动客户流失的核心因素。值得注意的是，Contract（合同类型）虽然在相关性分析中排名第一，但在SHAP模型归因中因与其他特征（如tenure、PaymentMethod）存在共线性而权重有所稀释。')
...
add_para('（1）在模型性能方面，随机森林在真实不平衡测试集上以ROC-AUC = 0.8386、Recall = 0.7086的综合表现成为最优模型。SMOTE过采样有效提升了模型对流失客户的识别能力，将Recall从无SMOTE的0.4893提升到0.7086。')
...
add_para('（4）在消融实验方面，SMOTE是召回率提升的关键，RFM与WOE提供了可解释的业务特征补充。')
...
add_para('所有实验均固定随机种子（random_state = 42），数据按80%/20%分层划分（stratify = y）。WOE仅在训练集上拟合，交叉验证阶段SMOTE通过Pipeline在每个训练折内执行，测试集保持原始分布。完整的Notebook和Python脚本见代码附件。运行命令为：pip install -r requirements.txt && python train.py --config configs/default.yaml。')
...
charts = [
    ...
    'cv_results.csv — 折内SMOTE交叉验证结果',
    'ablation_results.csv/png — 消融实验结果',
